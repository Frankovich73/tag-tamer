...
class redshift_cluster_resources_tags:
    ...
    def get_redshift_cluster_resources_tags(self, chosen_resources, **session_credentials):
        my_status = execution_status()
    ...
    def set_redshift_cluster_resources_tags(
        ...
        for resource_arn in self.resources_to_tag:
            try:
                ...
                my_status.success(message="RDS redshift DB Cluster tags updated successfully!")
            except botocore.exceptions.ClientError as error:
                log.error("Boto3 API returned error: {}".format(error))
